...
BOOL CDataGrid::Create(RECT wndRect, HWND hParent, int numCols)
{
     ...
     if ( ( reg ) || ( error == ERROR_CLASS_ALREADY_EXISTS ) )
     {
          ...
          m_hWnd = CreateWindowEx( WS_EX_CLIENTEDGE, szClassName, _T(""), WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_VSCROLL | WS_HSCROLL,
	...
	//To true?
          ...
          if ( dgList != NULL )
          {
               ...
               // Create EDIT control
				...
				EditProc = (WNDPROC)GetWindowLong(m_hEditWnd , GWL_WNDPROC);
				//SendMessage (m_hEditWnd ,EM_LIMITTEXT,-1,0);
				::SetWindowLong(m_hEditWnd , GWL_WNDPROC, (long)SubEditProc);
